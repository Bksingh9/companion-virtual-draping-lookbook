...
assert(currentHtml.includes("Play Video"), "Ready video state must expose a Play Video control");
assert(currentHtml.includes("Open Video"), "Ready video state must expose an Open Video control");
assert(!currentHtml.includes("Copy Video URI"), "Video URI copy control must stay out of the shopper flow");
qa.castCurrentLook();
